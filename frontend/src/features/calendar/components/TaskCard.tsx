...
const TaskCard = ({ item, index, handleChange }: any) => {
  ...
  return (
    <div
      key={index}
      className="h-[200px] w-[800px] min-w-[700px] p-3 mt-6 ml-6 items-center justify-between"
    >
      <Link to="">
        <div className="flex w-full px-2 justify-between align-center content-center">
          <div className="flex flex-col">
            <p className="mb-3 w-[100px]">{item.date}</p>
          ...
          <div className="flex flex-col w-full bg-white rounded-xl ml-12 px-10 pt-6 text-[18px] ">
            <p className="font-bold">{item.service.title}</p>
            <p className="font-bold mb-3 ">{item.pet.name}</p>
            <p className="">
              Dr. {item.doctor.name} - {item.doctor.email}
            </p>
